...
namespace foo_enhanced_playcount
{
	...
	// Called from init_stage_callback to hook ourselves into the metadb
	...
	{
	public:
		updateRecentScrobbles(const bool pullNew, metadb_handle_list_cref library) : newScrobbles(pullNew), allLibraryItems(library) {}
							...
							if (seenHashes.find(hash) == std::end(seenHashes)) {
								...
								if (!record.lastfmPlaytimes.size() || // if we don't have any scrobbles OR last known scrobble was more than 1 minute before scrobble_time
									((fileTimeWtoU(record.lastfmPlaytimes.back()) - 60) < s.scrobble_time) &&
										(s.scrobble_time - fileTimeWtoU(record.lastfmPlaytimes.back()) > 60))
								{ // filtering for non-adjusted recorded scrobbles
	...
	{
	public:
		get_lastfm_scrobble(const metadb_handle_ptr &handle, const bool refresh, const bool recent) : m_handle(handle), doRefresh(refresh), isRecent(recent) {}
	...
	void GetLastfmScrobblesThreaded(metadb_handle_list_cref items, bool always_show_popup)
	{
		...
		try {
			...
			if (items.get_count() == 1)
				threaded_process_flags &= ~threaded_process::flag_show_progress;
			if (always_show_popup)
				threaded_process_flags &= ~threaded_process::flag_show_delayed;
