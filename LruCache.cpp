...
template <typename K, typename V> void LruCache <K, V>::set(const K& k, const V& v) {
...
template <typename K, typename V> bool LruCache <K, V>::get(const K& k, V& v) {
	auto itr = cacheMap.find(k);
	bool ret = false;
...
template <typename K, typename V> bool LruCache <K, V>::erase(const K& k) {
	auto itr = cacheMap.find(k);
	bool ret = false;
